---
name: diagnose
description: Disciplined diagnosis loop for hard bugs and performance regressions. Use for bugs that are flaky, not reproducible, or require bisection — not for straightforward errors. Reproduce → minimise → hypothesise → instrument → fix → regression-test.
---
...
> **Scope:** Use this skill for hard bugs — flaky, not reproducible, or performance regressions needing bisection. For straightforward errors and test failures, the `debugger` subagent handles those faster.
...
10. **HITL script.** Last resort. If a human must click, drive _them_ with `scripts/hitl-loop.template.ps1` so the loop is still structured. Captured output feeds back to you.
...
If after 3 attempts the rate stays below ~10%, treat this as "cannot build a loop" and fall through to the section below.
...
- [ ] The failure is reproducible across multiple runs (or, for non-deterministic bugs, reproducible at a high enough rate to debug against — ≥ 10%, see Phase 1).
...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling), record a follow-up refactor task capturing the following:

- **Defect pattern**: what structural problem allowed this bug to exist (e.g. "no test seam at service boundary", "hidden coupling between X and Y")
- **Affected files**: paths of the modules involved
- **Root cause**: the hypothesis that proved correct
- **Suggested seam**: where an interface should be introduced to prevent recurrence
